...
| [Security Reporting Decision Rubric](./Frameworks/Security-Reporting-Decision-Rubric/) | Released | srdr-v1.0.0 (2026-08-14) | 4-question decision flow for designing audience-scoped security reports; severity floor guidance grounded in Defender XDR's severity model; 6 example templates (board quarterly, CISO monthly, board posture summary, exec committee quarterly, SOC lead weekly, audit attestation); a Board & Executive Policy Kit pairing a per-control policy translation worksheet with the board posture summary; Sentinel workbook and Security Copilot integration guide with a per-audience KQL library; executive business case |
...
| [Zero Trust Readiness Assessment](./Frameworks/Zero-Trust-Readiness-Assessment/) | Preview | ztra-v0.1.1-preview (2026-08-18) | 6-pillar M365 ZT posture assessment across 40 controls, aligned to CISA ZTMM v2.0 (Traditional → Optimal) with NIST SP 800-207 tenet citations; read-only Graph collector with throttle-aware retry and a paging ceiling; a failed call reports manual review rather than an absent control, and reports disclose when an assessment is partial; produces technical detail, executive summary, and board 1-pager outputs |
| [Identity Threat Protection Scorecard](./Frameworks/Identity-Threat-Protection-Scorecard/) | Preview | itps-v0.1.1-preview (2026-08-17) | 4-dimension scorecard (Prevention, Detection, Governance, Ownership) across 21 checks; read-only Graph collector with throttle-aware retry and an opt-in `-IncludeEvidence` mode; Detection requires sensor-deployment evidence before scoring, and Governance checks read scope filters rather than display names; 3 scenario examples plus a sample tenant report |
