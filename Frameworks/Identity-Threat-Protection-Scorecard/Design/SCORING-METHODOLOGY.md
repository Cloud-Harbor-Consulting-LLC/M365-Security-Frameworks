...
| P-02 MFA enforced for all users | 10 | A policy in `enabled` state that grants `mfa` or an `authenticationStrength`, includes **All users**, includes **All applications**, scopes no user action, and carries no risk condition |
...
| P-06 Phishing-resistant strength in use | 10 | A policy in `enabled` state requiring an authentication strength whose every allowed combination consists solely of phishing-resistant methods. `ManualReview` where a strength cannot be classified |
...
**P-02 measures blanket enforcement, not the presence of an MFA policy.** The check previously required only that a policy grant MFA and include All users. That matched three kinds of policy that do not enforce MFA generally: user-action policies covering *register security info* or *register or join device*, which set `conditions.applications.includeUserActions` and leave `includeApplications` empty; risk-conditional policies, which require MFA only above a risk threshold; and combinations of the two. On a live tenant it matched 4 policies where only 1 enforced blanket MFA — the score was correct there by luck, but a tenant holding only a register-device policy would have earned full marks with no general MFA requirement at all. The check now also requires All applications, no user-action scope, and no risk condition.

**P-06 measures phishing resistance, not the presence of an authentication strength.** The check previously matched any policy carrying an `authenticationStrength`, so the built-in *Multifactor authentication* strength — which permits SMS and Authenticator push — counted the same as a phishing-resistant one. Classification now reads `allowedCombinations` and requires that *every* permitted combination consist solely of `fido2`, `windowsHelloForBusiness`, or `x509CertificateMultiFactor`, because a strength is only as strong as its weakest allowed path. Where Graph does not return `allowedCombinations`, the built-in policy ids are used as a fallback: `00000000-0000-0000-0000-000000000004` is phishing-resistant, while `...0002` (Multifactor authentication) and `...0003` (Passwordless MFA) are not. Where neither signal resolves, P-06 reports `ManualReview` rather than scoring zero, since a wrong zero would understate a tenant that has done the work.
