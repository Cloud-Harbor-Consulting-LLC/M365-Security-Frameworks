...
- **ITPS Governance scoring, corrected against a live tenant.** A second live assessment — run after configuring a single access review — moved Governance from 22 to 57 and the overall score from 67 to 79. Most of that movement was unearned, and tracing it exposed 5 defects.
  - **G-02 scored a guest review that did not exist.** The matcher substring-matched `guest` against the review's *display name*, so a review named `CHC-Demo-NonGuest-Test-Access-Review` scored 15/15 — the name says the opposite of what was matched. Display names are free text, so any tenant could have earned the points by naming a review "guest". G-02 now reads the scope filter for `userType eq 'Guest'` and never the name. The scope shape also had to change: `scope.query` is null for a `principalResourceMembershipsScope`, the shape the Entra portal creates most often, which nests `principalScopes[]` and `resourceScopes[]` instead. A new `Get-ITPSScopeQuery` helper reads all of them. Where definitions exist but expose no readable filter, G-02 is now `ManualReview` rather than guessed; where no definitions exist at all, it remains a scored zero.
  - **G-01 awarded full marks for a single access review.** It was binary on count > 0, so one review was indistinguishable from a mature programme. Now graduated: 0 scores 0, 1 scores 10, 2 or more score 20 — full points at the 2-review baseline the EIG Toolkit ships (EIG-AR001, EIG-AR002).
  - **G-03 contradicted G-04 from identical data.** G-03 awarded a binary 20/20 whenever one eligible PIM assignment existed, while G-04 read the same assignments and awarded 2.5/25 for a 90% standing-privilege ratio — the scorecard reported exemplary PIM adoption and near-total standing privilege at once. G-03 is withdrawn and its points folded into G-04, now 45 points on that single axis. Governance drops from 5 checks to 4, and the scorecard from 22 checks to 21.
  - **G-05 was binary where G-04 was linear**, so 2 offending registrations out of 10 scored the same 0/20 as 10 out of 10. Now scaled linearly on the affected share. Renamed to `Workload identity credential lifetime` with an `AppsWithLongLivedSecrets` signal, because the predicate matches secrets that never expire or run past the threshold — not stale ones. Already-expired secrets are deliberately excluded: an expired credential cannot authenticate.
  - **Dimension scores lost precision and rounded the wrong way at midpoints.** `($earned / $available) * 100` evaluates 57.5 of 100 as `57.49999999999999`, reporting 57 where 58 was correct, and `[Math]::Round` defaults to banker's rounding, which sends 58.5 down to 58. Both the dimension and overall calculations now multiply before dividing and specify `MidpointRounding::AwayFromZero`.
  - **Detection scored 100/100 without evidence that anything was deployed.** An empty `healthIssues` collection is returned both by a tenant whose sensors are healthy and by a tenant with no sensors at all, and both earned full marks across D-01 to D-04 — so a board report could certify a detection capability that did not exist. D-04, described as establishing that Defender for Identity was "licensed and provisioned", established only that the endpoint answered. When the collection is empty the collector now requires positive deployment evidence from the `AATP_Sensor` Secure Score control, read from the `controlScores` payload already retrieved for P-01, so no additional request or scope is involved; without it, D-01 to D-04 report `ManualReview` and are excluded from the denominator. The similarly-named `AATP_DefenderForIdentityIsNotInstalled` control is deliberately not used: on a tenant with sensors installed on all 3 of its domain controllers it scored 0 with an empty `implementationStatus`, so reading it as an installed flag reports a fully deployed tenant as having no deployment. Where health issues are returned, sensors demonstrably exist and the evidence check is skipped.
  - Also hardened: G-04 now reports `ManualReview` when both PIM endpoints answer with no assignments at all, rather than scoring full marks. Every tenant holds at least one privileged assignment, so an empty set is an unreadable signal, not an absence of standing privilege.
  - Verified with a 30-assertion harness that extracts the shipped functions via AST rather than reimplementing them, covering all 3 scope shapes, the negated `userType ne 'Guest'` filter, every G-02 and Detection-gate branch, and the midpoint rounding cases. `SCORING-METHODOLOGY.md`, both READMEs, and all 4 worked examples are updated to match; `Sample-Tenant-Scorecard.md` was regenerated from the formatter rather than hand-edited, preserving its guarantee of being verbatim output.
- `Frameworks/Identity-Threat-Protection-Scorecard/Scripts/Format-ITPScorecardReport.ps1`: the executive summary listed unassessed checks as gaps, so Detection at 100/100 appeared to have a gap in D-05 — a check with no API that was never scored. Gaps and manual checks are now separate columns. The Fortified and Resilient tier descriptions no longer assert coverage the checks may contradict: the board report previously printed "Broad coverage across human and non-human identities" for a tenant whose only non-human identity check had scored zero.
- `Frameworks/Identity-Threat-Protection-Scorecard/Examples/Small-Business-Scenario.md`: corrected an internal contradiction — the narrative referred to a score of 34 while the scorecard above it reported 36.
- `Frameworks/Identity-Threat-Protection-Scorecard/Scripts/Get-ITPScorecard.ps1`: fixed the Secure Score request paging through the tenant's entire retained history. In Microsoft Graph, `$top` sets the page size rather than a result limit, so `security/secureScores?$top=1` returns one record per page plus a `nextLink`. The pager followed those links to exhaustion, issuing roughly one HTTP request per day of retained Secure Score history — on the order of 400 sequential requests — to obtain a single record that the very next line reduced with `Items[0]`. The run was slow enough to appear hung and long enough to cross a token-refresh boundary, which re-engaged the Windows Web Account Manager broker and produced repeated authentication prompts even though the script calls `Connect-MgGraph` exactly once. `Invoke-ITPSGraphRequest` and `Invoke-ITPSCollection` gain a `-FirstPageOnly` switch, used at the Secure Score call site, reducing that call from ~400 requests to 1. A generous `-MaxPages` ceiling (default 200) now also guards every paged call against a `nextLink` that never terminates, and reports truncation rather than passing over it, because a silently truncated collection would understate a score. Verified with a mocked Graph client counting requests: 1 request with the switch, correct multi-page accumulation without it. The Zero Trust Readiness Assessment collector does not call `secureScores` and is unaffected.
- `Frameworks/Identity-Threat-Protection-Scorecard/Scripts/Format-ITPScorecardReport.ps1`: fixed a crash when a dimension had no gaps. `Get-DimensionGap` returns a collection, and a function returning an empty collection unrolls to nothing, leaving the caller with `$null` and making `$gaps.Count` throw under `Set-StrictMode -Version Latest`. Any dimension whose checks were all at full points with no manual checks — a fully remediated Prevention dimension, for example — aborted report generation. Both call sites now wrap the result in `@()`. This is the same collection-unrolling root cause as the collector defect corrected in the previous entry, found in a different form while testing the `-TenantName` passthrough.
- `Frameworks/Identity-Threat-Protection-Scorecard/Scripts/Get-ITPScorecard.ps1`: fixed a scoring defect in which a successful Graph call returning **zero records** was treated as a failed call. Assigning a collection through a `try`/`catch` (`$x = try { @(fn) } catch { $null }`) yields `$null` when the result is empty, because PowerShell unrolls an empty collection to zero pipeline objects. Every affected check fell to `ManualReview` and was excluded from its dimension denominator, inflating the score of tenants whose controls were genuinely absent. Most severely, a tenant with **zero open Defender for Identity health issues** — the ideal state — had all of D-01 through D-04 excluded, leaving the Detection dimension unscored and indistinguishable from an unlicensed tenant. The same path affected Conditional Access policies (P-02 to P-06), access review definitions (G-01, G-02), PIM assignments (G-03, G-04), and application registrations (G-05). All 6 call sites now route through a new `Invoke-ITPSCollection` helper that reports success-with-data, success-but-empty, and failure as distinct outcomes: an empty result scores zero and counts in the denominator, and only a failed call is excluded. Confirmed against a live tenant assessment where zero access review definitions produced `ManualReview` instead of a scored zero, inflating Governance by 13 points and the overall score by 4.
- `Frameworks/Identity-Threat-Protection-Scorecard/Scripts/Get-ITPScorecard.ps1`: `ManualReviewNote` now reports the actual Graph error message instead of asserting an unverified cause. The previous notes stated a licensing requirement as the reason a call failed, which was a hypothesis presented as fact and made a genuine permissions failure indistinguishable from a licensing gap. Licensing is now named as a common cause to confirm, not a conclusion.
...
- `Frameworks/Security-Reporting-Decision-Rubric/Examples/Exec-Committee-Quarterly.md`: Executive Committee quarterly review template. Investment-approval and risk-acceptance decisions, top 3 material incidents, and 4 key metrics. Sits between the CISO Monthly and Board Quarterly.
...
- `Frameworks/Security-Reporting-Decision-Rubric/Design/SENTINEL-COPILOT-INTEGRATION.md`: Sentinel and Security Copilot integration guide. Sentinel workbook ARM template pattern, KQL query library per audience (SOC lead, CISO, executive committee, board), and Security Copilot prompt patterns for auto-drafting executive narrative sections.
- `Frameworks/Security-Reporting-Decision-Rubric/Business-Case/ROI-REPORTING-RUBRIC.md`: Executive business case. Cost of undifferentiated reporting (4 failure modes), kill-list ROI, audience-scoped cadence value, adoption cost and timeline, compliance alignment (SOC 2, ISO 27001:2022, NIST SP 800-53), cross-references to CA Baseline and EIG Toolkit business cases.

### Changed

- `Frameworks/Security-Reporting-Decision-Rubric/REPORTING-DECISION-RUBRIC.md`: severity floor section distinguishes alert severity from incident severity in Defender XDR; kill-list expanded from 6 to 9 entries (added Copilot activity summary, Agent ID sign-in count, Secure Score weekly change); SOC lead weekly metrics updated — Security Copilot auto-triage measurement definition clarified, Agent ID risk detection metric added.
- `Frameworks/Security-Reporting-Decision-Rubric/README.md`: status flipped from v0.1.0-preview to Released (v1.0.0); "What is in this repo" regrouped into Core rubric, Example templates, Integration guide, and Business case — listing all 6 templates and surfacing the `Design/` and `Business-Case/` content that shipped in earlier PRs but was never listed; "How to use it" extended from 4 to 6 steps to cover Sentinel workbooks and Security Copilot prompt patterns.
- Root `README.md`: SRDR Frameworks table row updated to Released / v1.0.0.

### Fixed

- `Frameworks/Security-Reporting-Decision-Rubric/Examples/Audit-Attestation.md`: corrected SOC 2 CC control mappings — CC6.7 replaced with CC6.6 (logical access security against external threats); CC7.1 description corrected to reflect configuration and vulnerability monitoring; MTTD moved from CC7.1 into CC7.2 (anomaly detection indicative of malicious acts). Source: Azure + Dynamics 365 + Online Services SOC 2 Type II Report (2025-04-01 to 2026-03-31).
...
[Unreleased]: <https://github.com/Cloud-Harbor-Consulting-LLC/M365-Security-Frameworks/compare/srdr-v1.0.0...HEAD>

[srdr-v1.0.0]: <https://github.com/Cloud-Harbor-Consulting-LLC/M365-Security-Frameworks/compare/itps-v0.1.0-preview...srdr-v1.0.0>
